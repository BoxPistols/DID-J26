...
|---------|--------|---------|---------|--------|
| **飛行注意区域（DID）** | e-Stat（総務省統計局） | [e-Stat](https://www.e-stat.go.jp/) | 5年ごと | 🔴 最高 |
...
- ✅ **ビューポートベースの動的読み込み**: クライアント側で表示範囲内の都道府県のみを自動的に読み込む（パフォーマンス向上）
- ✅ **地方ごとの分類表示**: 47都道府県すべてを一度に読み込むと画面が重くなるため、地方ごとに分類し、必要な地域だけを選択して表示可能
...
**A:** 飛行注意区域（DID）データは以下の特性があるため、静的配置が最適です。また、パフォーマンス向上のため、地方ごとに分類されており、必要な地域だけを選択して表示することを推奨します。47都道府県すべてを一度に読み込むと、大量のデータ（数万〜数十万のポリゴン）がメモリに読み込まれ、GPU/CPU/メモリを急激に消費して画面が重くなります。
...
- 飛行注意区域（DID）は統計データのため、最新の市街地変化とずれる場合あり
- DIDの表示方法：地方ごとに分類されているのは、パフォーマンス向上のためです。全国一括表示モードでは、ビューポートベースの動的読み込みにより、表示範囲内の都道府県のみを自動的に読み込むため、パフォーマンスが改善されています
