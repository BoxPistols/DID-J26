...
**全国天気マップ**
- 全47都道府県 + 地域区分（北海道4地域、沖縄3地域）の天気を一覧表示
- TV天気予報スタイルの天気アイコンと気温表示
- クリックで詳細ポップアップ（風速、湿度、降水量）
- グラスモーフィズムUIで背景地図も視認可能
- ESCキーでポップアップを閉じる
- ダークモード対応
...
- 全47都道府県のDIDデータを表示
- **地方ごとの分類表示**：パフォーマンス向上のため、地方ごとに分類されています。47都道府県すべてを一度に読み込むと、大量のデータ（数万〜数十万のポリゴン）がメモリに読み込まれ、GPU/CPU/メモリを急激に消費して画面が重くなります。
- **ビューポートベースの動的読み込み**：全国一括表示モードでは、表示範囲内の都道府県のみを自動的に読み込むため、パフォーマンスが改善されています。
- **推奨使用方法**：必要な地域だけを選択して表示することで、快適に動作します。各地域グループから必要な都道府県を個別に選択することで、必要なデータだけを読み込み、メモリ使用量とレンダリング負荷を最小限に抑えられます。
- 透明度調整
- ホバー時のポップアップ情報（人口、面積、人口密度、市区町村コード）
- 一括表示/非表示切り替え
- **地方ごとのDID+NFZセット表示モード**：各地域グループから「DID+NFZ表示」ボタンをクリックすると、その地域のDIDと空港空域（NFZ）を同時に表示できます。
...
| 機能 | 説明 |
|------|------|
| **座標表示** | クリック位置の座標を表示・クリップボードにコピー |
| **座標形式切替** | 10進数（35.6812°）⇔ 60進数（35°40'52"）の切替 |
| **天気予報** | その場所の天気予報を表示 |
| **規制エリア表示** | DID、空港周辺、レッド/イエローゾーンの表示切替 |
| **UI設定** | サイドバー表示、ダークモード等の切替 |
...
| ゾーン | 色 | 深刻度 |
|--------|-----|--------|
| 飛行注意区域（DID） | 赤系 | WARNING（許可取得で飛行可能） |
...
- **ビューポートベースの動的読み込み**：表示範囲内の都道府県のみを自動的に読み込むことで、メモリ使用量とレンダリング負荷を削減
- **地方ごとの分類表示**：必要な地域だけを選択して表示することで、パフォーマンスを向上
...
- **飛行注意区域（DID）**: 国勢調査の人口集中地区（e-Stat）に基づく統計データです。更新周期が長く、最新の市街地変化とずれる場合があります。
- **DIDの表示方法**: 地方ごとに分類されているのは、パフォーマンス向上のためです。47都道府県すべてを一度に読み込むと、大量のデータ（数万〜数十万のポリゴン）がメモリに読み込まれ、GPU/CPU/メモリを急激に消費して画面が重くなります。地域別表示では必要な地域のみを読み込むため軽量です。全国一括表示はビューポートベースの動的読み込みにより、表示範囲内の都道府県のみを自動的に読み込むため、パフォーマンスが改善されていますが、広域表示時は重くなる可能性があります。
...
データ読み込み中は画面最上部にプログレスバーが表示されます。

### 表示条件（トリガー）

以下のデータ読み込み時にプログレスバーが表示されます：

| データ種別 | トリガー | 進捗表示 |
|-----------|---------|---------|
| **DIDレイヤー** | レイヤー表示ON時 | あり（読み込み完了で終了） |
| **NFZ（空港空域）** | レイヤー表示ON時 | あり |
| **施設データ** | レイヤー表示ON時 | あり |
| **全国天気マップ** | 天気マップ表示ON時 | あり（0-100%） |
| **地方一括読み込み** | DID+NFZ表示ボタン押下時 | あり |

### 終了条件

- **正常終了**: データ読み込み完了後、0.3秒のフェードアウトアニメーションで消える
- **エラー時**: 読み込み失敗時も終了し、エラートーストが表示される
- **複数同時読み込み**: すべてのデータが読み込み完了するまでバーは表示され続ける

### UI仕様

- **位置**: 画面最上部（高さ4px）
- **アニメーション**: グラデーションがスライドする視覚的フィードバック
- **テキスト表示**: 画面右上に読み込み中のレイヤー名と進捗率（%）を控えめに表示
- **スピナー**: 読み込み中はテキスト横に回転するスピナーアイコン
- **点滅**: テキストが軽く点滅して読み込み中であることを視覚的に強調
