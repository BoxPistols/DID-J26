...
**ビューポートベースの動的読み込み:**
- 全国一括表示モードでは、表示範囲内の都道府県のみを自動的に読み込むため、パフォーマンスが改善されています
- マップの移動・ズーム時に自動的に必要なデータのみを読み込む（デバウンス300ms）
- 各都道府県のバウンディングボックス情報を事前に定義し、ビューポートと交差する都道府県のみを読み込む

**推奨使用方法:**
- 必要な地域だけを選択して表示することで、快適に動作します
- 各地域グループから必要な都道府県を個別に選択することで、必要なデータだけを読み込みます
