...
```typescript
// OK:改善後: requestAnimationFrameで節流（最大 60 Hz）
...
```typescript
// OK:改善後: バッチ処理 + requestIdleCallback
const BATCH_SIZE = 7
let batchIndex = 0
...
```typescript
// NG:改善前: 全国一括表示時に全47都道府県を一度に読み込む
...
<table style={TableStyle.table}>
  ...
  <tbody>
    <tr style={TableStyle.tr}>
      ...
      <td style={TableStyle.td}>NG 固まる</td>
      <td style={TableStyle.td}>OK スムーズ</td>
    </tr>
    <tr style={TableStyle.tr}>
