...
- **`sources`**: データの取得元を定義（例: タイルURL、GeoJSONパス）
- **`layers`**: データの描画方法を定義（例: 色、透明度、線幅）
- **`glyphs`**: テキストラベルのフォント（例: PBF形式フォントのURL）
- **`sprite`**: アイコン画像セット（例: スプライト画像のURL）
...
- **`line`**: 線の描画（LineString, Polygon境界線）
- **`circle`**: 点を円で表示（Point）
...
- **layout**: 変更コスト高い（再レイアウト）。例: visibility, text-size
- **paint**: 変更コスト低い（再描画のみ）。例: fill-color, opacity
...
- **`interpolate`**: 中間値を滑らかに補間。サイズや色の段階的変化に使用
- **`step`**: 閾値で離散的に変化。表示/非表示の切り替えに使用
...
- **ラスタータイル**: PNG/JPG画像。背景地図に使用
- **ベクタータイル（MVT）**: バイナリ形式。大規模ポリゴン/ラインに適する
